...
{
    [LuaUsage]
    public sealed class Fixed64
    {
        public static Fixed64 Num2Fixed64(double value)
        {
            return new Fixed64(value);
        }

        public static Fixed64 Str2Fixed64(string value)
        {
            var s = (ulong)0;
            ...
            if (values.Length == 0 || values.Length > 2){
                throw new Exception("Fixed64: " + value + " format is err ");
             }
            if (values.Length == 1){
            ...
            var di = new Fixed64(Double.Parse(values[0]));
            var de= new Fixed64(Double.Parse(values[1]));


            var dee = new Fixed64(Math.Pow(10,values[1].Length));
            return new Fixed64((ulong)(di.Add(de.Div(dee)).v) | s);
        }
        ...
        public Fixed64(double value)
        {
            ...
            if (realE >= 0)
            {
                ...
                if (pBitsNum < 0)
                {
                    ...
                    {
                        throw new System.Exception("Fixed number: part digital overflow");
                    }
        ...
        public Fixed64 Add(Fixed64 f2)
        {
            var ret = new Fixed64();
            var f1 = new Fixed64(v);
            var fS = f1.v & mask64S;
            var oS = f2.v & mask64S;
            ...
            if (fS == oS)
            {
                var addRes = Add64(f1.v, f2.v);
                if ((addRes.Item1 >> 31) > 0)
                {
                    throw new Exception("FixedNumber: Add overflow " + f1.ToBase10N(18) + " " + f2.ToBase10N(18));
                }
                ret.v = ((addRes.Item1 << 32) + addRes.Item2) | fS;
                return ret;
            }
            else
            {
        ...
        public Fixed64 Sub(Fixed64 f2)
        {
            f2.v = f2.v ^ mask64S;
            return Add(f2);
        }

        public Fixed64 Mul(Fixed64 f2)
        {
            Fixed64 f1 = new Fixed64(v);
            var fS = f1.v & mask64S;
            var oS = f2.v & mask64S;
            f1.v = bitClear(f1.v, mask64S);
            f2.v = bitClear(f2.v, mask64S);
            ...
            {
                throw new Exception("Fixed64: Number OverFlow");
            }
        ...
        public Fixed64 Abs()
        {
            return new Fixed64(bitClear(v, mask64S));
        }
        ...
        public static Tuple<ulong, ulong> Add64(ulong x, ulong y)
        {
            var x0 = x >> 32;
            var x1 = x & mask32;
            var y0 = y >> 32;
